...
void drawBitmapTrans(Bitmap *bmp, int x, int y, Alignment alignment, int flg) {
  if (bmp == NULL)
    return;
  ...
  for (i = 0; i < height; i++) {
    ...
    bufferStartPos = (unsigned char *) getDrawingBuffer();
    bufferStartPos += x * 2 + pos * getHorResolution() * 2;

    imgStartPos = (unsigned char *) bmp->bitmapData + xCorrection * 2 + i * width * 2;
    ...
    for (j = 0; j < drawWidth * 2; j += 2) {
      ...
      if (imgStartPos[j] != 0x1f && imgStartPos[j + 1] != 0xf8) 
      {
        if (flg == 1) 
        {
          if (bufferStartPos[j] == 0x00 && bufferStartPos[j + 1] == 0x00)
              gameOver = true;
        }
