...
int(timer_set_frequency)(uint8_t timer, uint32_t freq)
{
  ...
  util_get_MSB(value, &_MSB);
  util_get_LSB(value, &_LSB);
  ...
  case 0:
    ...
    sys_outb(TIMER_0, _LSB);
    sys_outb(TIMER_0, _MSB);

  case 1:
    byte1 = (byte1 & 0x0f) | TIMER_1 | TIMER_LSB_MSB;
    sys_outb(TIMER_CTRL, byte1);
    sys_outb(TIMER_1, _LSB);
    sys_outb(TIMER_1, _MSB);

  case 2:
    byte1 = (byte1 & 0x0f) | TIMER_2 | TIMER_LSB_MSB;
    sys_outb(TIMER_CTRL, byte1);
    sys_outb(TIMER_2, _LSB);
    sys_outb(TIMER_2, _MSB);
  }
